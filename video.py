        ...
        if len(recs)>0:
            ...
            for rec in recs:
                ...
                for i, b in enumerate(binary_blocks):
                    ...
                    if  flag:
                        ...
                        #print(centre.flatten().shape)
                if sum(digits_flag) >= 17: #if valid digits in the sudoku is less than 17 then we think it as invalid
                    valid = True
                    invalid_cnt = 0
                    #because it's valid, we'll set it to invalid_cnt to 0 || Invalid hone ke lie, 3 consecutive invalid!
            ...
            if not valid:
                ...
                if valid_cnt > 10:#连续１０帧有效，才能确保检测到一个谜题了，如果中间有连续三帧断了那么需要重新来，valid_cnt会被打断重置成零，如果只是一帧断了那么下次还能继续。
                    ...
                    #print(pre_centre_blocks[0].shape) # (784,)
                    #cv2.waitKey(0)
                    pred_labels = pred_func(pre_centre_blocks)+1
                    #pred_labels = predictDigit(model, pre_centre_blocks)
                    print(pred_labels)
                    sudoku = np.zeros(81)
